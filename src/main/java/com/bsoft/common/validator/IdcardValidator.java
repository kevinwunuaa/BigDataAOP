...
public class IdcardValidator extends AbstractValidator{
	...
	public JSONObject validate(String value) {
		...
		if (value.length() == 15) {
			success = validate15IDCard(value);
		}else{
			success = validate18Idcard(value);
		}
		
		if(!success){
			json.put("success", false);
			json.put("info", String.format("身份证号[%s]验证不通过！", value));
		}
	...
	public static void main(String[] args) throws Exception {
		...
		// 15位身份证
		System.out.println(v.validate(idcard15));
		// 18位身份证
		System.out.println(v.validate(idcard18));
		// 15位身份证转18位身份证
